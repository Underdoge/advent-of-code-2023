...
def count_steps(pipes: list, start: (int, int)) -> int:
    ...
    width = len(pipes[0])
    steps = 0
    def flood_fill_pipes(x: int, y: int, dir: str, steps: int) -> int:
        ...
        if x == start[1] and y == start[0]:
            ...
            match dir:
                case 'up':
                    if pipes[y][x] == '|':
                        return flood_fill_pipes(x, y-1, 'up', steps+1)
                    elif pipes[y][x] == 'F':
                        return flood_fill_pipes(x+1, y, 'right', steps+1)
                    elif pipes[y][x] == '7':
                        return flood_fill_pipes(x-1, y, 'left', steps+1)
                case 'down':
                    if pipes[y][x] == '|':
                        return flood_fill_pipes(x, y+1, 'down', steps+1)
                    elif pipes[y][x] == 'L':
                        return flood_fill_pipes(x+1, y, 'right', steps+1)
                    elif pipes[y][x] == 'J':
                        return flood_fill_pipes(x-1, y, 'left', steps+1)
    ...
    if y-1 >= 0 and pipes[y-1][x] in ['|', 'F', '7']:
        steps = flood_fill_pipes(x, y-1, 'up', 1)
    elif y+1 <= height-1 and pipes[y+1][x] in ['|', 'L', 'J']:
        steps = flood_fill_pipes(x, y+1, 'down', 1)
    elif x-1 >= 0 and pipes[y][x-1] in ['-', 'F', 'L']:
        steps = flood_fill_pipes(x-1, y, 'left', 1)
    elif x+1 <= width-1 and pipes[y][x+1] in ['-', 'J', '7']:
        steps = flood_fill_pipes(x+1, y, 'right', 1)
    return steps
